...
| [src/acp.ts](../src/acp.ts) | Provider-neutral ACP client — spawns the selected backend, manages session lifecycle, normalizes through its backend hooks, and emits the extension's established events. `interject` (#52 Steer), `forkSession` (#48), `submitFeedback` (#114), and worktree RPCs (P2-8) call the unadvertised `_x.ai/*` methods, returning `"unsupported"` on -32601 rather than throwing |
| [src/feedback.ts](../src/feedback.ts) | Pure thumbs-feedback helpers (#114) — availability, snake_case params, no `turn_number` (agent attributes the current turn); see [research/turn-feedback.md](../research/turn-feedback.md) |
