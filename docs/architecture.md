...
| [src/session.ts](../src/session.ts) | Per-session state bag — one `Session` per live backend process, with immutable `provider` identity (the sidebar holds a *pool* plus one focused); carries the send queue (#37, per-item `{text, chips}` via `queued-send.ts`), exclusive history-load join (`runExclusiveHistoryLoad` / `loadInFlight`), and optional worktree binding (`cwd` / `worktree`), while cumulative billing stays solely in session-id-keyed metadata (#53) |
