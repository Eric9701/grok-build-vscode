...
- **Cloning uses the Git credentials you already have.** No token to paste, nothing stored, and public repositories need nothing at all. When a private one fails, the form says so and offers the next step rather than printing what Git said: **Sign in to GitHub** runs `gh auth login` in a terminal, or, if the GitHub CLI is not installed, offers to install it and names the exact command first.
