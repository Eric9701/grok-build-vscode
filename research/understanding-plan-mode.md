...
- Workspace **file** writes are not all extension-enforced. On grok 1.x the client withholds `readTextFile`, writes are not delegated, and Plan-mode file safety rests on grok's native edit refusal. On 0.2.x the delegated handshake still applies, so every `fs/write_text_file` whose target resolves inside the workspace cwd is refused by the extension. The only write that is deliberately allowed on that path is grok writing its own plan to `~/.grok/sessions/<...>/plan.md`; the extension *snoops* that write as a fallback. Plan review is fed by `req.plan` (`exit_plan_mode.planContent`) when the CLI sends it (1.x); 0.2.117 still sends `null`.
